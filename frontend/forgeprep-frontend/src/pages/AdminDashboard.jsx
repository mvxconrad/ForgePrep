...
const AdminDashboard = () => {
  ...
  const fetchAdminData = async () => {
    try {
      ...
      console.log("Admin Data:", response.data); // ✅ Debugging log
  ...
  useEffect(() => {
    const verifyAdmin = async () => {
      try {
        await checkAdmin();
        setLoading(false); // ✅ Now properly updates loading after admin check
      } catch (err) {
        console.error("Error verifying admin:", err);
        setLoading(false); // Avoid infinite spinner even on failure
      }
    };
